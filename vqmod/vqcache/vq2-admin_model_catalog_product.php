	  ...
	  if (isset($data['def_img']) && $data['def_img'] != "" && $this->config->get('pim_miu')) {
       $q="UPDATE " . DB_PREFIX . "product SET image = '" . $this->db->escape($data['def_img']) . "' WHERE product_id = '" . (int)$product_id . "'";
		...
		if ($this->config->get('config_auto_generate_seo_url')) {
			$query = $this->db->query("SELECT pd.product_id, pd.name, pd.language_id ,l.code FROM " . DB_PREFIX . "product p inner join " . DB_PREFIX . "product_description pd ON p.product_id = pd.product_id inner join " . DB_PREFIX . "language l on l.language_id = pd.language_id WHERE p.product_id = '" . (int)$product_id . "'");

			foreach ($query->rows as $product_row) {
				if (strlen($product_row['name']) > 1) {
					$slug = generateSlug($product_row['name']);
					...
					if (!$exist_query->num_rows) {
						...
						if ($exist_keyword->num_rows) { 
							...
							if ($exist_keyword_lang->num_rows) {
								$slug = generateSlug($product_row['name']).'-'.rand();
							} else {
								$slug = generateSlug($product_row['name']).'-'.$product_row['code'];
							}
						}
