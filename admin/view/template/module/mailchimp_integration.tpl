	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<?php if (!$autosave) { ?>
					<a onclick="saveSettings($(this))" class="btn btn-primary"><i class="fa fa-floppy-o pad-right"></i> <?php echo $button_save; ?></a>
					<a onclick="location = '<?php echo $exit; ?>'" class="btn btn-default"><i class="fa fa-reply pad-right"></i> <?php echo $button_back; ?></a>
				<?php } ?>
				<?php if ($permission && $settings_buttons) { ?>
			...
			<ul class="breadcrumb">
				...
				<li><a href="<?php echo $exit; ?>"><?php echo ${'standard_'.$type}; ?></a></li>
				<li><a href="index.php?route=<?php echo $type . '/' . $name; ?>&token=<?php echo $token; ?>"><?php echo $heading_title; ?></a></li>
			</ul>
		</div>
	</div>
	...
	<div class="container-fluid">
		<form id="form" action="" class="form-horizontal autosave" autocomplete="off">
			...
			<?php foreach ($settings as $setting) { ?>
						...
						<?php if (isset($setting['buttons'])) { ?>
							<div class="pull-right">
								<?php if ($setting['buttons'] == 'expand_collapse') { ?>
									...
									(parent);" class="btn btn-default" data-help='<?php echo $help_expand_all; ?>'><i class="fa fa-caret-square-o-down pad-right"></i> <?php echo $button_expand_all; ?></a>
				...
				$all_attributes .= ($default) ? 'data-default="' . $default . '" ' : '';
				...
				<?php if ($setting['type'] == 'button') { ?>
					...
					<?php } elseif ($key == 'delete') { ?>
						<a class="btn btn-danger" data-help='<?php echo $button_delete; ?>' onclick="<?php if ($autosave) echo "if (confirm('" . $standard_confirm . "'))"; ?> removeRow($(this).parents('tr'))"><i class="fa fa-trash-o fa-lg fa-fw"></i></a>
					<?php } ?>
				...
				<?php } elseif ($setting['type'] == 'multilingual_text' || $setting['type'] == 'multilingual_textarea') { ?>
					...
					<?php foreach ($language_array as $language_code => $language_name) { ?>
						...
						<div class="input-group" <?php if ($help_text) echo "data-help='" . $help_text . ' ' . $language_name . "'"; ?>>
							...
							<?php echo ($text) ? '<input type="text"' : '<textarea'; ?> class="form-control <?php echo $class; ?>" placeholder="<?php if (isset(${'placeholder_'.$base_key})) echo $language_name . ' ' . ${'placeholder_'.$base_key}; ?>" name="<?php echo $key . '_' . $language_code; ?>" <?php echo $attributes; ?> <?php echo ($text) ? " value='" : '>'; ?><?php echo str_replace("'", "&apos;", $value); ?><?php echo ($text) ? "' />" : '</textarea>'; ?>
						</div>
					<?php } ?>
				...
				<?php } elseif ($setting['type'] == 'rule') { ?>
					...
					<?php foreach (array_merge(array(0), $setting['rules']) as $rule) { ?>
						...
						<div class="rule" <?php if ($rule == 0) echo 'style="display: none"'; ?>>
							...
							<?php if (!empty($rule_options['adjustments'])) { ?>
								...
								<?php if (!empty($setting_override_array)) { ?>
									<span class="setting_override-html" <?php if ($rule_type != 'setting_override') echo 'style="display: none"'; ?>>
										<select class="form-control" name="<?php echo $key . '_' . $rule; ?>_comparison">
										<option value=""><?php echo $standard_select; ?></option>
											<?php $groupcode = (version_compare(VERSION, '2.0.1') < 0) ? 'group' : 'code'; ?>
							...
							<?php if (in_array('shipping_cost', $rule_options['order_criteria']) && $type != 'shipping') { ?>
							...
							<?php if (in_array('shipping_rate', $rule_options['order_criteria'])) { ?>
				...
				<?php } elseif ($setting['type'] == 'typeahead') { ?>
							...
							<select class="nosave form-control" style="display: inline-block" data-help='<?php echo $help_autocomplete_from; ?>'>
...
<script type="text/javascript"><!--
	...
	<?php } else { ?>
		function saveSettings(element) {
			element.attr('disabled', 'disabled').html('<i class="fa fa-refresh fa-spin pad-right"></i> <?php echo $standard_saving; ?>');
			$.ajax({
				type: 'POST',
				url: 'index.php?route=<?php echo $type; ?>/<?php echo $name; ?>/saveSettings&token=<?php echo $token; ?>',
				data: $('#form :input:not(:checkbox), #form :checkbox:checked'),
				success: function(error) {
					element.addClass('btn-success').removeAttr('disabled').html('<i class="fa fa-check pad-right"></i> <?php echo $standard_saved; ?>');
					if (error) alert(error);
					setTimeout(function(){
						element.removeClass('btn-success').html('<i class="fa fa-floppy-o pad-right"></i> <?php echo $button_save; ?>');
					}, 2000);
				},
				error: function(jqXHR, textStatus, errorThrown) {
					element.removeAttr('disabled').html('<i class="fa fa-floppy-o pad-right"></i> <?php echo $button_save; ?>');
	...
	function attachTooltips(element) {
		...
		element.find('*[data-help]').each(function(){
			if ($('input[name="tooltips"]').val() == 1) {
				$(this).attr('title', $(this).attr('data-help')).tooltip({'animation': false, 'placement': ($(this).attr('data-help').length < 400) ? 'auto top' : 'auto right', 'html':true});
			} else {
