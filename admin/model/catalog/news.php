...
class ModelCatalogNews extends Model {
	public function addNews($data, $ifcopy = 0) {
		...
		if ($this->config->get('config_auto_generate_seo_url')) {
			$query = $this->db->query("SELECT nd.news_id, nd.title, nd.language_id, l.code FROM " . DB_PREFIX . "sb_news n inner join " . DB_PREFIX . "sb_news_description nd on n.news_id = nd.news_id inner join " . DB_PREFIX . "language l on l.language_id = nd.language_id WHERE n.news_id = '" . (int)$news_id . "'");
						
			foreach ($query->rows as $news_row) {
				if(strlen($news_row['title']) > 1) {
					$slug = generateSlug($news_row['title']);

					$exist_query = $this->db->query("SELECT query FROM " . DB_PREFIX . "url_alias WHERE " . DB_PREFIX . "url_alias.query = 'news_id=" . $news_row['news_id'] . "'");
					...
					if (!$exist_query->num_rows) {
						...
						if ($exist_keyword->num_rows) { 
							$exist_keyword_lang = $this->db->query("SELECT query FROM " . DB_PREFIX . "url_alias WHERE " . DB_PREFIX . "url_alias.keyword = '" . $slug . "' AND " . DB_PREFIX . "url_alias.query <> 'news_id=" . $news_row['news_id'] . "'");

							if ($exist_keyword_lang->num_rows) {
								$slug = generateSlug($news_row['title']).'-'.rand();
							} else {
								$slug = generateSlug($news_row['title']).'-'.$news_row['code'];
							}
						}	
						
						$add_query = "INSERT INTO " . DB_PREFIX . "url_alias (query, keyword) VALUES ('news_id=" . $news_row['news_id'] . "', '" . $slug . "')";
