...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		if ($this->config->get('config_auto_generate_seo_url')) {
			$query = $this->db->query("SELECT id.information_id, id.title, id.language_id, l.code FROM " . DB_PREFIX . "information i inner join " . DB_PREFIX . "information_description id on i.information_id = id.information_id inner join " . DB_PREFIX . "language l on l.language_id = id.language_id WHERE i.information_id = '" . (int)$information_id . "'");
						
			foreach ($query->rows as $info_row) {
				if(strlen($info_row['title']) > 1) {
					$slug = generateSlug($info_row['title']);
					...
					if (!$exist_query->num_rows) {
						...
						if ($exist_keyword->num_rows) { 
							...
							if ($exist_keyword_lang->num_rows) {
								$slug = generateSlug($info_row['title']).'-'.rand();
							} else {
								$slug = generateSlug($info_row['title']).'-'.$info_row['code'];
							}
						}	
