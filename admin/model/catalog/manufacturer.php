...
class ModelCatalogManufacturer extends Model {
	public function addManufacturer($data) {
		...
		if ($this->config->get('config_auto_generate_seo_url')) {
			$query = $this->db->query("SELECT name, manufacturer_id FROM " . DB_PREFIX . "manufacturer WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");

			foreach ($query->rows as $manufacturer_row) {
				if (strlen($manufacturer_row['name']) > 1) {
					$slug = generateSlug($manufacturer_row['name']);
					...
					if (!$exist_query->num_rows) {
						...
						if ($exist_keyword->num_rows) {
							$slug = generateSlug($manufacturer_row['name']).'-'.rand();
						}
